...
%
These natural composites are distinguished from their synthetic counterparts by the facts that their effective properties at the macro scale, such as strength and fracture toughness, are quite high despite the corresponding properties of their constituent \setulcolor{Red}\ul{phases are poor.}\todo[fancyline]{English. "being very poor"}
%
...
%\begin{equation}
%\begin{aligned}
%\sigma_{rr} & = (\kappa_x \sin\theta - \kappa_y \cos\theta)\left( \sum_{i \in \mathcal{J}} K_i r^{m_i-1} + \mu_1 r \right) \\&+ \sum_{i=1}^{2} K'_i r^{m'_i-1} + \mu_3 \vartheta r + \mu_5 \epsilon \\
%\sigma_{\theta\theta} & = (\kappa_x \sin\theta - \kappa_y \cos\theta)\left( \sum_{i \in \mathcal{J}} K_i (m_i+1) r^{m_i-1} + 3\mu_1 r \right) \\&+ \sum_{i=1}^{2} K'_i m'_i r^{m'_i-1} + 2\mu_3 \vartheta r + \mu_5 \epsilon \\
%\tau_{r\theta} & = (\kappa_x \cos\theta + \kappa_y \sin\theta) \left( -\sum_{i \in \mathcal{J}} K_i r^{m_i-1} - \mu_1 r \right) \\
%\tau_{rz} & = (\kappa_x \cos\theta + \kappa_y \sin\theta) \left( \sum_{i \in \mathcal{J}} K_i g_i r^{m_i-1} + \mu_2 r \right) \\
%\tau_{\theta z} & =  (\kappa_x \sin\theta - \kappa_y \cos\theta)\left( -\sum_{i \in \mathcal{J}} K_i g_i m_i r^{m_i-1} - 2\mu_2 r \right) \\&- \sum_{i=1}^{2} K'_i g'_i r^{m'_i-1} - \mu_4 \vartheta r - \left( \mu_5 \frac{\beta_{14}+\beta_{24}}{\beta_{44}} + \frac{C_{34}}{C_{33}\beta_{44}} \right) \epsilon \\
...
\begin{equation}
	\begin{aligned}
	EI = & \sum_{n=1}^{N} \left[ \sum_{i \in \mathcal{J}} {}^{n}\!{\alpha_{i}} {}^{n}\!{K_{i}}  ({}^{n-1}\!{r}^{{}^{n}\!{m_{i}}+2} - {}^{n}\!{r}^{{}^{n}\!{m_{i}}+2}) + \gamma_n({}^{n-1}\!{r}^4 - {}^{n}\!{r}^4) \right],
	\end{aligned}
	\label{eq:EI}
\end{equation}
with
\begin{subequations}
	...
	\gamma_n & := \frac{\pi}{{}^{n}\!{C_{33}}} \frac{ {}^{n}\!{\mu_{1}} ({}^{n}\!{C_{13}} + 3{}^{n}\!{C_{23}}) - 2{}^{n}\!{\mu_{2}} {}^{n}\!{C_{34}} - 1  }{4},
	\end{align}
\end{subequations}
where~$N$ denotes the total number of layers,~$\mathcal{J} := \pr{1,2,3,4}$,~${}^{n-1}\!{r}$ and~${}^{n}\!{r}$ the internal and external radii of the~$n^\text{th}$ layer,~${}^{n}\!{m_{i}}$,~${}^{n}\!{g_{i}}$,~${}^{n}\!{\mu_{1}}$, and~${}^{n}\!{\mu_{2}}$ are material constants related with elastic constants of the~$n^\text{th}$ layer (see~\ref{Appen:MatConst}).
...
\begin{subequations}
	\begin{align}
	\sum_{i \in \mathcal{J}} {}^{1}\!{K_{i}} {}^{0}\!{r}^{{}^{1}\!{m_{i}}-2} & = -{}^{1}\!{\mu_{1}}, \quad
	\sum_{i \in \mathcal{J}} {}^{1}\!{K_{i}} {}^{1}\!{g_{i}} {}^{0}\!{r}^{{}^{1}\!{m_{i}}-2} = -{}^{1}\!{\mu_{2}},\\
	\sum_{i \in \mathcal{J}} {}^{N}\!{K_{i}} {}^{N}\!{r}^{{}^{N}\!{m_{i}}-2} & = -{}^{N}\!{\mu_{1}}, \quad
	\sum_{i \in \mathcal{J}} {}^{N}\!{K_{i}} {}^{N}\!{g_{i}} {}^{N}\!{r}^{{}^{N}\!{m_{i}}-2} = -{}^{N}\!{\mu_{2}}.
	\end{align}
	\label{eq:boundary}
\end{subequations}
...
\begin{subequations}
	...
	\sum_{i \in \mathcal{J}} {}^{n}\!{K_{i}} {}^{n}\!{r}^{{}^{n}\!{m_{i}}-2} & = -{}^{n}\!{\mu_{1}}, \quad
	\sum_{i \in \mathcal{J}} {}^{n}\!{K_{i}} {}^{n}\!{g_{i}} {}^{n}\!{r}^{{}^{n}\!{m_{i}}-2} = -{}^{n}\!{\mu_{2}}, \\
	\sum_{i \in \mathcal{J}} {}^{n}\!{K_{i}} {}^{n-1}\!{r}^{{}^{n}\!{m_{i}}-2} & = -{}^{n}\!{\mu_{1}}, \quad
	\sum_{i \in \mathcal{J}} {}^{n}\!{K_{i}} {}^{n}\!{g_{i}} {}^{n-1}\!{r}^{{}^{n}\!{m_{i}}-2} = -{}^{n}\!{\mu_{2}},
	\end{align}
...
\begin{subequations}
	\begin{align}
	& \sum_{i \in \mathcal{J}} \left( {}^{n}\!{K_{i}} r^{{}^{n}\!{m_{i}}-2} - {}^{n+1}\!{K_{i}} r^{{}^{n+1}\!{m_{i}}-2} \right) = {}^{n+1}\!{\mu_{1}} 
  - {}^{n}\!{\mu_{1}}, \\
	& \sum_{i \in \mathcal{J}} \left( {}^{n}\!{K_{i}} {}^{n}\!{g_{i}} r^{{}^{n}\!{m_{i}}-2} - {}^{n+1}\!{K_{i}} {}^{n+1}\!{g_{i}} r^{{}^{n+1}\!{m_{i}}-2} \right) = {}^{n+1}\!{\mu_{2}} - {}^{n}\!{\mu_{2}}, \\
	& \sum_{i \in \mathcal{J}} \left( {}^{n}\!{K_{i}} {}^{n}\!{Q_{i}} r^{{}^{n}\!{m_{i}}-2} - {}^{n+1}\!{K_{i}} {}^{n+1}\!{Q_{i}} r^{{}^{n+1}\!{m_{i}}-2} \right) = {}^{n+1}\!{Q_{5}} - {}^{n}\!{Q_{5}}, \\
	& \sum_{i \in \mathcal{J}} \left( {}^{n}\!{K_{i}} {}^{n}\!{W_{i}} r^{{}^{n}\!{m_{i}}-2} - {}^{n+1}\!{K_{i}} {}^{n+1}\!{W_{i}} r^{{}^{n+1}\!{m_{i}}-2} \right) = {}^{n+1}\!{W_{5}} - {}^{n}\!{W_{5}},
	\end{align}
...
\begin{equation}
	{}^{n}\!{K_{i}} = P_i \cdot {}^{n}\!{r}^{-m_i + 2} + O(\Delta r), \quad \text{for}\, i \in \mathcal{J},
...
\begin{equation}
	\begin{aligned}
	EI & = \sum_{n=1}^{N} \left[ \sum_{i \in \mathcal{J}} \alpha_i \left( P_i {}^{n}\!{r}^{-m_i + 2} + O(\Delta r) \right) ({}^{n-1}\!{r}^{{}^{n}\!{m_{i}}+2} - {}^{n}\!{r}^{m_{i}+2}) \right] + \gamma ({}^{0}\!{r}^4 - {}^{N}\!{r}^4) \\
	 & \begin{split}
		= \sum_{n=1}^{N} \left[ \sum_{i \in \mathcal{J}} \alpha_i \left( P_i {}^{n}\!{r}^{-m_i + 2} + O(\Delta r) \right) (- {}^{n}\!{r}^{m_{i}+1}(m_i + 2 ) \Delta r + O(\Delta r^2)) \right] \\
		+ \gamma ({}^{0}\!{r}^4 - {}^{N}\!{r}^4)
	   \end{split}\\
	 & = - \sum_{n=1}^{N} \left[ \sum_{i \in \mathcal{J}} \left( \alpha_i P_i (m_i + 2 ) {}^{n}\!{r}^{3} \cdot \Delta r \right) \right] + \gamma ({}^{0}\!{r}^4 - {}^{N}\!{r}^4) + O(\Delta r^2).
	\end{aligned}
...
\begin{equation}
	\begin{aligned}
	(EI)_{\infty} & = - \lim\limits_{N \to \infty} \sum_{n=1}^{N} \left[ \sum_{i \in \mathcal{J}} \alpha_i P_i (m_i + 2 ) {}^{n}\!{r}^{3} \Delta r \right] + \gamma ({}^{0}\!{r}^4 - {}^{N}\!{r}^4) + O(\Delta r^2) \\
	 & = - \sum_{i \in \mathcal{J}} \left( \int_{{}^{0}\!{r}}^{{}^{N}\!{r}} \alpha_i P_i (m_i + 2 ) r^3 \, dr \right) + \gamma ({}^{0}\!{r}^4 - {}^{N}\!{r}^4) \\
	 & = - \frac{{}^{N}\!{r}^4 - {}^{0}\!{r}^4}{4} \left[ \sum_{i \in \mathcal{J}} \alpha_i P_i (m_i + 2 ) + 4\gamma\right].
	\end{aligned}
...
\begin{equation}
	{}^{n}\!{K_{i}} = {}^{n}\!{P_{i}} \cdot {}^{n}\!{r}^{-{}^{n}\!{m_{i}} + 2} + O(\Delta r), \quad \text{for}\, i \in \mathcal{J},
...
\begin{equation}
	\begin{aligned}
	\begin{split}
	EI & = \sum_{n=1}^{N} \bigg[ \sum_{i \in \mathcal{J}} {}^{n}\!{\alpha_{i}} \left( {}^{n}\!{P_{i}} {}^{n}\!{r}^{-{}^{n}\!{m_{i}} + 2} + O(\Delta r) \right) \cdot \\ & \qquad{} (- {}^{n}\!{r}^{{}^{n}\!{m_{i}}+1}({}^{n}\!{m_{i}} + 2 ) \Delta r + O(\Delta r^2))  + \gamma_n (-4 {}^{n}\!{r}^3 \Delta r + O(\Delta r^2)) \bigg] \\
	 & = -\sum_{n=1}^{N} \bigg[ \sum_{i \in \mathcal{J}} {}^{n}\!{\alpha_{i}} {}^{n}\!{P_{i}} ({}^{n}\!{m_{i}} + 2) + 4\gamma_n \bigg] {}^{n}\!{r}^3 \cdot \Delta r + O(\Delta r^2).
	\end{split}
	\end{aligned}
	\label{eq:two_mat_no_friction_EI_1}
\end{equation}
Since the arrangement of material property is periodic, the odd layers share the same material constants~${}^{I}\!{\alpha_{i}}$,~${}^{I}\!{P_{i}}$,~${}^{I}\!{m_{i}}$, and~$\gamma_{I}$ while the even layers share the same material constants~${}^{II}\!{\alpha_{i}}$,~${}^{II}\!{P_{i}}$,~${}^{II}\!{m_{i}}$, and~$\gamma_{II}$. We rewrite Eqn.~\eqref{eq:two_mat_no_friction_EI_1} as
\begin{equation}
	\begin{aligned}
		EI & = -\sum_{\text{odd layers}} \bigg[ \sum_{i \in \mathcal{J}} {}^{I}\!{\alpha_{i}} {}^{I}\!{P_{i}} ({}^{I}\!{m_{i}} + 2) + 4\gamma_{I} \bigg] {}^{n}\!{r}^3 \cdot \Delta r \\ & \quad{} -\sum_{\text{even layers}} \bigg[ \sum_{i \in \mathcal{J}} {}^{II}\!{\alpha_{i}} {}^{II}\!{P_{i}} ({}^{II}\!{m_{i}} + 2) + 4\gamma_{II} \bigg] {}^{n}\!{r}^3 \cdot \Delta r + O(\Delta r^2) \\
		 & = -\frac{1}{2}
		 \bigg\lbrace
		 \sum_{n=1}^N \bigg[ \sum_{i \in \mathcal{J}} {}^{I}\!{\alpha_{i}} {}^{I}\!{P_{i}} ({}^{I}\!{m_{i}} + 2) + 4\gamma_{I} \bigg] {}^{n}\!{r}^3 \cdot \Delta r \\ & \quad\qquad{} + \sum_{n=1}^N \bigg[ \sum_{i \in \mathcal{J}} {}^{II}\!{\alpha_{i}} {}^{II}\!{P_{i}} ({}^{II}\!{m_{i}} + 2) + 4\gamma_{II} \bigg] {}^{n}\!{r}^3 \cdot \Delta r
...
\begin{equation}
	 \begin{aligned}
	   (EI)_{\infty} & = \frac{1}{2}  \Biggl \{
	   		- \Biggl[ \sum_{i \in \mathcal{J}} {}^{I}\!{\alpha_{i}} {}^{I}\!{P_{i}} ({}^{I}\!{m_{i}} + 2) + 4\gamma_{I} \Biggr]
			\int_{{}^{0}\!{r}}^{{}^{N}\!{r}} r^3 \, dr \\ & \quad\qquad{}
			- \Biggl[ \sum_{i \in \mathcal{J}} {}^{II}\!{\alpha_{i}} {}^{II}\!{P_{i}} ({}^{II}\!{m_{i}} + 2) + 4\gamma_{II} \Biggr]
			\int_{{}^{0}\!{r}}^{{}^{N}\!{r}} r^3 \, dr
	    \Biggl \} \\
...
where
\begin{equation}
	(EI)_{j,\infty} = - \frac{{}^{N}\!{r}^4 - {}^{0}\!{r}^4}{4} \left[ \sum_{i \in \mathcal{J}} {}^{j}\!{\alpha_{i}} {}^{j}\!{P_{i}} ({}^{j}\!{m_{i}} + 2 ) + 4\gamma_j\right], \quad \text{for} \, j = I, II.
\end{equation}
...
For the case that the lamination scheme of the multilayered cylindrical composite is~$[\varphi_I/\varphi_{II}]_{N/2}$ under no slip interfacial condition, the~$4N$ unknowns~${}^{n}\!{K_{i}}$ can be solved from the boundary conditions by Eqn.~\eqref{eq:boundary} and the interfacial conditions by Eqn.~\eqref{eq:noslip_interface}, which can be rewritten as
\begin{equation}
\begin{aligned}
& \begin{bmatrix}
...
We first substitute~$n$ as~$n-1$ into Eqn.~\eqref{eq:eqs of Kin_two_mat_no_slip_np1} and combine the resulting equation with Eqn.~\eqref{eq:eqs of Kin_two_mat_no_slip_np1} to find out the relation between~$\usf{K}_{n}$ and~${}^{{n-2}}{\usf{K}}$ as
\begin{equation}
...
\begin{equation}
	2 {}^{n}\!{r} \frac{d (\usf{r}_{n,n}{}^{n}{\usf{K}})}{dr} = \left( {}^{n}\tilde{\usf{m}} + {}^{n}\tilde{\usf{m}} \right) \usf{r}_{n,n}{}^{n}{\usf{K}} + r^2_n {}^{n}{\usf{F}},
...
\begin{equation}
%{}^{n}\!{K_{i}} = {}^{n}\!{r}^{-{}^{n}\!{m_{i}}} \left( G^0_{i,n} r^2_n + G^1_{i,n} r^{{}^{n}\!{\lambda_{1}}}_n + G^2_{i,n} r^{{}^{n}\!{\lambda_{2}}}_n + G^3_{i,n} r^{{}^{n}\!{\lambda_{3}}}_n + G^4_{i,n} r^{{}^{n}\!{\lambda_{4}}}_n \right)
{}^{n}\!{K_{i}} = {}^{n}\!{r}^{-{}^{n}\!{m_{i}}} \left( G^0_{i,n} r^2_n + \sum_{j \in \mathcal{J}} G^j_{i,n} r^{\lambda_{j,n}}_n \right), \quad \text{for}\, i \in \mathcal{J},
...
\begin{equation}
\begin{aligned}
	EI & = - \sum_{n=1}^{N} \left\lbrace \sum_{i \in \mathcal{J}} {}^{n}\!{\alpha_{i}}' \left( G^0_{i,n} r^2_n + \sum_{j \in \mathcal{J}} G^j_{i,n} r^{\lambda_{j,n}}_n \right) {}^{n}\!{r} + 4\gamma_n {}^{n}\!{r}^3 \right\rbrace + O(\Delta r) \\
	& =  - \sum_{n=1}^{N} \left\lbrace \left( \sum_{i \in \mathcal{J}} {}^{n}\!{\alpha_{i}}' G^0_{i,n} + 4\gamma_n \right) {}^{n}\!{r}^3+ \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} {}^{n}\!{\alpha_{i}}' G^j_{i,n} r^{1 + \lambda_{j,n}}_n \right\rbrace + O(\Delta r),
\end{aligned}
\end{equation}
where~${}^{n}\!{\alpha_{i}}' := {}^{n}\!{\alpha_{i}} \left( {}^{n}\!{m_{i}} + 2\right)$.

Since the material properties of the layers of the structure are alternately arranged, the odd layers share the same material constants~${}^{I}\!{\alpha_{i}}$,~$\gamma_{I}$,~${}^{I}\!{m_{i}}$,~$G^j_{i,I}$, and~$\lambda_{j,I}$, and the even layers have the same constants~${}^{II}\!{\alpha_{i}}$,~$\gamma_{II}$,~${}^{II}\!{m_{i}}$,~$G^j_{i,II}$, and~$\lambda_{j,II}$. Therefore,
\begin{equation}
\begin{aligned}
	EI & =  - \frac{1}{2} \sum_{n=1}^{N} \left[ \left( \sum_{i \in \mathcal{J}} {}^{I}\!{\alpha_{i}}' G^0_{i,I} + 4\gamma_I \right) {}^{n}\!{r}^3+ \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} {}^{I}\!{\alpha_{i}}' G^j_{i,I} r^{1 + \lambda_{j,I}}_n \right] \\
	 & \quad - \frac{1}{2} \sum_{n=1}^{N} \biggr[ \left( \sum_{i \in \mathcal{J}} {}^{II}\!{\alpha_{i}}' G^0_{i,II} + 4\gamma_{II} \right) {}^{n}\!{r}^3 \\
	  & \qquad \qquad \qquad \qquad + \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} {}^{II}\!{\alpha_{i}}' G^j_{i,II} r^{1 + \lambda_{j,II}}_n \biggr] + O(\Delta r).
\end{aligned}
\end{equation}
As~$N \to \infty$, we obtain the asymptotic bending stiffness as
\begin{equation}
\begin{aligned}
	(EI)_{\infty} & =  - \frac{1}{2} \left[ \sum_{i \in \mathcal{J}} \left( {}^{I}\!{\alpha_{i}}' G^0_{i,I} + {}^{II}\!{\alpha_{i}}' G^0_{i,II} \right) + 4\left( \gamma_I +\gamma_{II} \right) \right] \int_{{}^{0}\!{r}}^{{}^{N}\!{r}} r^3 \, dr \\
	& \quad - \frac{1}{2} \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} \left[ \int_{{}^{0}\!{r}}^{{}^{N}\!{r}} \left( {}^{I}\!{\alpha_{i}}' G^j_{i,I} r^{1 + \lambda_{j,I}} + {}^{II}\!{\alpha_{i}}' G^j_{i,II}  r^{1 + \lambda_{j,II}} \right) \, dr \right] \\
	& = - \frac{1}{8} \left[ \sum_{i \in \mathcal{J}} \left( {}^{I}\!{\alpha_{i}}' G^0_{i,I} + {}^{II}\!{\alpha_{i}}' G^0_{i,II} \right) + 4\left( \gamma_I +\gamma_{II} \right) \right] \left( {}^{N}\!{r}^4 - {}^{0}\!{r}^4 \right) \\
	& \quad - \frac{1}{2} \sum_{i \in \mathcal{J}} \sum_{j \in \mathcal{J}} \biggr[ \frac{{}^{I}\!{\alpha_{i}}' G^j_{i,I}}{2 + \lambda_{j,I} } \left( {}^{N}\!{r}^{2 + \lambda_{j,I}} - {}^{0}\!{r}^{2 + \lambda_{j,I}} \right) \\
	& \qquad \qquad \qquad \quad + \frac{{}^{II}\!{\alpha_{i}}' G^j_{i,II}}{2 + \lambda_{j,II}} \left( {}^{N}\!{r}^{2 + \lambda_{j,II}} - {}^{0}\!{r}^{2 + \lambda_{j,II}} \right) \biggr] .
\end{aligned}
...
\begin{equation}
	{}^{n}{\usf{A}} = {}^{n}\tilde{\usf{A}}  \usf{r}_{n-1,n}, \quad
	{}^{n}{\usf{B}} = {}^{n}\tilde{\usf{B}}  \usf{r}_{n-1,n-1}.
\end{equation}
Approximating~${}^{{n-1}}{\usf{A}}$ and~${}^{{n-1}}{\usf{B}}$ to the first order of~$\Delta r$ around~${}^{n-1}\!{r}$, we have
\begin{subequations}
\begin{align}
	{}^{{n-1}}{\usf{A}} & = \usf{B}_{n} - \delta \usf{B}_{n} \Delta r + O(\Delta r^2), \\
	{}^{{n-1}}{\usf{B}} & = \usf{A}_{n} - \delta \usf{A}_{n} \Delta r + O(\Delta r^2),
\end{align}
\end{subequations}
where
\begin{subequations}
	\begin{align}
	\delta \usf{A}_{n} & = \frac{1}{{}^{n-1}\!{r}} \usf{A}_{n} {}^{n}\tilde{\usf{m}} = \frac{1}{{}^{n-1}\!{r}} {}^{n}\tilde{\usf{A}} \usf{r}_{n-1,n} {}^{n}\tilde{\usf{m}}, \\
	\delta \usf{B}_{n} & = \frac{1}{{}^{n-1}\!{r}} \usf{B}_{n} {}^{n-1}\tilde{\usf{m}} = \frac{1}{{}^{n-1}\!{r}} {}^{n}\tilde{\usf{B}} \usf{r}_{n-1,n-1} {}^{n-1}\tilde{\usf{m}}.
	\end{align}
\end{subequations}
Therefore,
\begin{subequations}
	...
	\begin{split}
		{}^{n}{\usf{B}} {}^{{n-1}}{\usf{A}}^{-1}
		& = {}^{n}{\usf{B}} \left[ \usf{B}_{n} - \delta \usf{B}_{n} \Delta r + O(\Delta r^2) \right]^{-1} \\
		& = {}^{n}{\usf{B}} \left[ \usf{B}_{n} \left( \usf{I}- \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n-1}\tilde{\usf{m}} \right) + O(\Delta r^2) \right]^{-1} \\
		& = {}^{n}{\usf{B}} \left[ \usf{I} - \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n-1}\tilde{\usf{m}} + O(\Delta r^2) \right]^{-1} \usf{B}^{-1}_{n} \\
		& = \usf{I} + \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n}{\usf{B}}\  {}^{n-1}\tilde{\usf{m}}\  {}^{n}{\usf{B}}^{-1} + O(\Delta r^2),
	\end{split}\\
	\begin{split}
		{}^{n}{\usf{B}} {}^{{n-1}}{\usf{A}}^{-1} {}^{{n-1}}{\usf{B}}
		& = \left[ \usf{I} + \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n}{\usf{B}} {}^{n-1}\tilde{\usf{m}} {}^{n}{\usf{B}}^{-1} + O(\Delta r^2) \right] \cdot \\
		& \qquad \left[ \usf{A}_{n} - \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n}{\usf{A}} {}^{n}\tilde{\usf{m}} + O(\Delta r^2) \right] \\
		& = \usf{A}_{n} + \frac{\Delta r}{{}^{n-1}\!{r}} \left( {}^{n}{\usf{B}} {}^{n-1}\tilde{\usf{m}} {}^{n}{\usf{B}}^{-1} \usf{A}_{n} - {}^{n}{\usf{A}} {}^{n}\tilde{\usf{m}} \right) + O(\Delta r^2),
	\end{split} \\
	\begin{split}
		{}^{n}{\usf{B}} {}^{{n-1}}{\usf{A}}^{-1} {}^{{n-1}}{\usf{D}} + {}^{n}{\usf{D}}
		& = \left[ \usf{I} + \frac{\Delta r}{{}^{n-1}\!{r}} {}^{n}{\usf{B}} {}^{n-1}\tilde{\usf{m}} {}^{n}{\usf{B}}^{-1} + O(\Delta r^2) \right] {}^{{n-1}}{\usf{D}} + {}^{n}{\usf{D}} \\
...
\begin{equation}
	{}^{n}{\usf{B}} {}^{n-1}\tilde{\usf{m}} {}^{n}{\usf{B}}^{-1} = \tilde{\usf{B}}_n {}^{n-1}\tilde{\usf{m}} \tilde{\usf{B}}^{-1}_n,
\end{equation}
we have
\begin{equation}
\begin{aligned}
	\begin{split}
	2 {}^{n}\!{r} \frac{d {}^{n}{\usf{K}}}{dr} = & \left( \usf{r}_{n,n}^{-1} \tilde{\usf{A}}^{-1}_n \tilde{\usf{B}}_n {}^{n-1}\tilde{\usf{m}} \tilde{\usf{B}}^{-1}_n \tilde{\usf{A}}_n \usf{r}_{n,n} - {}^{n}\tilde{\usf{m}} \right) {}^{n}{\usf{K}} \\
	& + {}^{n}\!{r}^2 \usf{r}^{-1}_{n,n} \tilde{\usf{A}}^{-1}_n \left( -2\usf{I} + \tilde{\usf{B}}_n {}^{n-1}\tilde{\usf{m}} \tilde{\usf{B}}^{-1}_n \right) \tilde{\usf{D}}_n + O(\Delta r),
	\end{split}
\end{aligned}
...
\begin{subequations}
	\begin{align}
	{}^{n}\tilde{\usf{m}} & := \tilde{\usf{A}}^{-1}_n \tilde{\usf{B}}_n {}^{n-1}\tilde{\usf{m}} \tilde{\usf{B}}^{-1}_n \tilde{\usf{A}}_n, \\
	{}^{n}{\usf{F}} & := \tilde{\usf{A}}^{-1}_n \left( -2\usf{I} + \tilde{\usf{B}}_n {}^{n-1}\tilde{\usf{m}} \tilde{\usf{B}}^{-1}_n \right) \tilde{\usf{D}}_n,
	\end{align}
\end{subequations}
We rewrite Eqn.~\eqref{eq:AppendKndr2} as
\begin{equation}
	2 {}^{n}\!{r} \frac{d {}^{n}{\usf{K}}}{dr} = \left( \usf{r}_{n,n}^{-1} {}^{n}\tilde{\usf{m}} \usf{r}_{n,n} + {}^{n}\tilde{\usf{m}} \right) {}^{n}{\usf{K}} + {}^{n}\!{r}^2 \usf{r}_{n,n}^{-1} {}^{n}{\usf{F}}.
	\label{eq:AppendKndr}
\end{equation}
Then multiplying Eqn.~\eqref{eq:AppendKndr} by~$\usf{r}_{n,n}$ on both sides, we obtain a linear ODE system of~${}^{n}{\usf{K}}$
\begin{equation}
	2 {}^{n}\!{r} \frac{d \pr{\usf{r}_{n,n}{}^{n}{\usf{K}}}}{dr} = \pr{{}^{n}\tilde{\usf{m}} + {}^{n}\tilde{\usf{m}}} \pr{\usf{r}_{n,n}{}^{n}{\usf{K}}} + r^2_n {}^{n}{\usf{F}},
	\label{eq:AppendKn}
\end{equation}
...
\begin{equation}
	2 {}^{n}\!{r} \frac{d{}^{n}{\usf{K}}^e}{dr} = {}^{n}\tilde{\usf{m}}^e {}^{n}{\usf{K}}^e + r^2_n {}^{n}{\usf{F}},
	\label{eq:AppendKendr}
\end{equation}
where~${}^{n}{\usf{K}}^e := \usf{r}_{n,n}{}^{n}{\usf{K}}$ and~${}^{n}\tilde{\usf{m}}^e := {}^{n}\tilde{\usf{m}} + {}^{n}\tilde{\usf{m}}$. with boundary conditions
\begin{subequations}
	\begin{align}
	\sum_{i \in \mathcal{J}} K^e_{i,1} {}^{0}\!{r}^{-2} & = -{}^{1}\!{\mu_{1}}, \quad
	 \sum_{i \in \mathcal{J}} K^e_{i,1} {}^{1}\!{g_{i}} {}^{0}\!{r}^{-2} = -{}^{1}\!{\mu_{2}},\\
	\sum_{i \in \mathcal{J}} K^e_{i,N} {}^{N}\!{r}^{-2} & = -{}^{N}\!{\mu_{1}}, \quad
	 \sum_{i \in \mathcal{J}} K^e_{i,N} {}^{N}\!{g_{i}} {}^{N}\!{r}^{-2} = -{}^{N}\!{\mu_{2}}.
	\end{align}
	\label{eq:Appenboundary}
\end{subequations}

Since the material properties of the multilayered material is alternately arranged, the constant coefficient matrix~${}^{n}\tilde{\usf{m}}^e$ and~${}^{n}{\usf{F}}$ for the odd (and even) layers are the same and are denoted as~$\usf{M}_I^e$,~$\usf{F}_I$ (and~$\usf{M}_{II}^e$,~$\usf{F}_{II}$). In the asymptotic sense ($N \to \infty$),~${}^{n}{\usf{K}}$ of odd and even layers are continuous functions of the radius~$r$ (see Fig.~\ref{fig:two_material_no_slip_Ks}). Then Eqn.~\eqref{eq:AppendKendr} becomes
\begin{subequations}
\begin{align}
	2 r \frac{d{}^{n}{\usf{K}}^e}{dr} & = \usf{M}_I^e {}^{n}{\usf{K}}^e + r^2 \usf{F}_I, \quad \text{for odd~$n$}, \\
	2 r \frac{d{}^{n}{\usf{K}}^e}{dr} & = \usf{M}_{II}^e {}^{n}{\usf{K}}^e + r^2 \usf{F}_{II}, \quad \text{for even~$n$}.
\end{align}
...
Therefore,~${}^{n}{\usf{K}}^e$ of the odd and even layers are
\begin{subequations}
\begin{align}
{}^{n}{\usf{K}}^e & = \left( \usf{M}_I^{\nu} \right)^{-1}\usf{Y}_I, \quad \text{for odd~$n$}, \\
{}^{n}{\usf{K}}^e & = \left( \usf{M}_{II}^{\nu} \right)^{-1}\usf{Y}_{II}, \quad \text{for even~$n$},
\end{align}
\end{subequations}
where
\begin{equation}
...
\end{equation}
The integration constants~${}^{n}\!{c_{i}}$ are determined by the boundary condition given by Eqn.~\eqref{eq:Appenboundary}.
% and interfacial condition
% \begin{equation}
% 	\tilde{\usf{A}}_n {}^{n}{\usf{K}}^e = \tilde{\usf{B}}_n {}^{{n-1}}{\usf{K}}^e + r^2 \tilde{\usf{D}}_n.
% \end{equation}
Finally, we obtain~${}^{n}{\usf{K}}$ of the odd and even layers given by
\begin{subequations}
\begin{align}
	{}^{n}{\usf{K}} & = (\usf{r}_{n,n})^{-1} (\usf{M}_I^{\nu})^{-1}\usf{Y}_I, \quad \text{for odd~$n$}, \\
	{}^{n}{\usf{K}} & = (\usf{r}_{n,n})^{-1} (\usf{M}_{II}^{\nu})^{-1}\usf{Y}_{II}, \quad \text{for even~$n$}.
\end{align}
