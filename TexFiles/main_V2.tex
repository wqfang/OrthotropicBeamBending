...
\todo{\href{https://hackmd.io/-Q9bMN5xRDqp2RmYz8p-Ag}{HK to WF}}
...
%
\setulcolor{green}\textcolor{red}{For example, the tensile strength of the spicules of the sponge \textit{Euplectella aspergillum} is much higher than the silicate glass fiber~\cite{walter2007mechanisms}.}\todo[fancyline]{hk to WF: I think we should not refer to the paper by Walter for the tensile strength of EA spicules. I think SK will be able to advise you regarding this aspect.}
%
...
\begin{figure}[t]
  ...
  \graphicspath{{../LyxFiles/figure/}}
   \includegraphics[width=0.8\textwidth]{Cylinder3D_V3.pdf}
...
Substituting Eqn.~\eqref{eq:knknm2appr2} into Eqn.~\eqref{eq:knknm2appr1} and making simplification (see~\ref{Appen:GovernEqnKn} for details), we obtain a linear ordinary differential equation (ODE) system of~$\busf{K}(r)$
\begin{equation}
...
\subsection{Definitions of $m_{n\cdot i}$, where $i\in (1,2,3,4)$, and $n\in(1,\ldots,N)$}
\label{sec:mni}
By definition
\begin{subequations}
	\begin{align}
...
\begin{subequations}
  ...
  \label{eqns:anbncn}
\end{subequations}
The symbols $\betasub{n}{ij}$, which appear in \eqref{eqns:anbncn}, are called the reduced elastic constants and are defined such that
\begin{equation}
\betasub{n}{ij} = \texttt{HK to WF: Fill this up using $\Ssub{n}{ijkl}$}
% C_{ij}-\frac{C_{i3} C_{3j}}{C_{33}}.
\end{equation}

%hk done with section sec:mni

\subsection{Definitions of $\gsub{n}{i}$, where $i\in (1,2,3,4)$, and $n\in(1,\ldots,N)$}
\label{sec:gni}
...
\begin{equation}
	\begin{aligned}
		~^{n-1}\!{r} \frac{d ~^{n}{\busf{K}(r)}}{dr} = & \left( \Inv\pr{{}^{n}\usf{A}} ~^{n}{\usf{B}} ~^{n-1}\tilde{\usf{m}} \Inv\pr{{}^{n}\usf{B}} ~^{n}{\usf{A}} - ~^{n}\tilde{\usf{m}} \right) ~^{n}{\busf{K}(r)} \\
...
\begin{aligned}
	\begin{split}
	2 ~^{n}\!{r} \frac{d ~^{n}{\busf{K}(r)}}{dr} = & \left( \Inv\pr{{}^{n}\usf{f}({}^{n}\!r)} \Inv\pr{{}^{n}\tilde{\usf{A}}} ~^{n}\tilde{\usf{B}} ~^{n-1}\tilde{\usf{m}} \Inv\pr{{}^{n}\tilde{\usf{B}}} \tilde{\usf{A}}_n ~^{n}\usf{f}({}^{n}\!r) - ~^{n}\tilde{\usf{m}} \right) ~^{n}{\busf{K}(r)} \\
...
\begin{equation}
	2 ~^{n}\!{r} \frac{d ~^{n}{\busf{K}(r)}}{dr} = \left( \Inv\pr{{}^{n}\usf{f}({}^{n}\!r)} ~^{n}\tilde{\usf{M}} ~^{n}\usf{f}({}^{n}\!r) + ~^{n}\tilde{\usf{m}} \right) {}^{n}{\busf{K}(r)} + \p\pr{{}^{n}\!{r},2} \Inv\pr{{}^{n}\usf{f}({}^{n}\!r)} ~^{n}\tilde{\usf{F}}.
	\label{eq:AppendKndr}
\end{equation}

For simplicity, we drop the left superscript in all variables in the following derivations when there is no confusion. We can rewrite Eqn.~\eqref{eq:AppendKndr} as
\begin{equation}
  2 r \frac{d \busf{K}(r)}{dr} = \left( \Inv\pr{\usf{f}(r)} \tilde{\usf{M}} \usf{f}(r) + \tilde{\usf{m}} \right) \busf{K}(r) + \p\pr{r,2} \Inv\pr{\usf{f}(r)} \tilde{\usf{F}}.
  \label{eq:AppendKndrSimple}
\end{equation}


Then multiplying Eqn.~\eqref{eq:AppendKndrSimple} by~$\usf{f}(r)$ on both sides, we obtain a linear ODE system of~$\busf{K}(r)$
\begin{equation}
	2 r \frac{d \pr{\usf{f}(r)\busf{K}(r)}}{dr} = \pr{\tilde{\usf{M}} + \tilde{\usf{m}}} \pr{\usf{f}(r)\busf{K}(r)} + \p\pr{r,2} \tilde{\usf{F}},
	\label{eq:AppendKn}
\end{equation}
